...
export class RuleBasedParser {
  ...
  private extractTransaction(
    ...
    if (invM) {
      ...
      // Require full word "invoice" (prevents "inv" matching inside longer words)
      // and digit-start capture (invoice numbers are numeric, not "OICE" from INVOICE).
      // ::: Code Generated by Copilot 4a2f9d3e-5b7c-4e1a-8f0d-3c6b9e2d1a4f. This comment will be removed automatically after the file is saved :::
      const invHeaderRx = /^invoice\s+(\d[\w\-/]{2,20})$/im;
  ...
  private extractFromColumnTable(lines: string[]): DataLiftPart[] {
    ...
    for (let i = effectiveHeaderIdx + 1; i < lines.length; i++) {
      ...
      // Sanity guard: reject items whose extended total exceeds $100,000.
      // Lines from PO tables, order-number rows, or reference numbers can produce
      // multi-million-dollar "totals" that are obviously not product line amounts.
      // ::: Code Generated by Copilot 4a2f9d3e-5b7c-4e1a-8f0d-3c6b9e2d1a4f. This comment will be removed automatically after the file is saved :::
