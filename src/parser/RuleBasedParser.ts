...
import type {
  ...
  DataLiftPaymentDetails,
  DataLiftDeliveryDetails,
...
export class RuleBasedParser {
  parse(
    ...
    // ── Payment / Delivery / Notes (enrichment) ───────────────────────────────
    const paymentDetails: DataLiftPaymentDetails | undefined =
      extractPaymentDetails(normText);
    const deliveryDetails: DataLiftDeliveryDetails | undefined =
      extractDeliveryDetails(normText);
    const notes: string | undefined = extractNotes(normText);

    // ── Metadata ─────────────────────────────────────────────────────────────
    // Count non-empty top-level fields for fieldCount metric
    const countFields = (obj: Record<string, unknown>): number =>
      Object.values(obj).filter(
        (v) => v !== undefined && v !== null && v !== "",
      ).length;

    const fieldCount =
      countFields(supplier as unknown as Record<string, unknown>) +
      countFields(buyer as unknown as Record<string, unknown>) +
      countFields(transaction as unknown as Record<string, unknown>) +
      countFields(totals as unknown as Record<string, unknown>) +
      parts.length;
    ...
    if (paymentDetails) result.paymentDetails = paymentDetails;
    if (deliveryDetails) result.deliveryDetails = deliveryDetails;
  ...
  private segmentDocument(lines: string[]): {
    ...
    // NOTE: bare "total" is intentionally excluded here — "Total" can appear in
    // company names (e.g. "Total Maintenance Solutions"). Use grand/sub-total
    // specifics instead; bare "total" in footer is caught by footerKeywords.
    const bodyStartKeywords =
      /^(?:description|item|qty|quantity|part\s*(?:no|#)|sku|unit\s*price|amount|sub[\s\-]*total|grand\s*total|bill\s*to|ship\s*to|customer|product|service|particular|rate|no\.?\s|sr\.?\s*no)/i;
  ...
  private extractSupplierName(headerLines: string[]): string | undefined {
    ...
    for (const line of candidates) {
      ...
      if (/^[A-Z][A-Z\s&.,'-]{2,}$/.test(line)) return line.trim();
    }
  ...
  private extractBuyer(_fullText: string, lines: string[]): DataLiftBuyer {
    ...
    // Negative lookahead prevents "CUSTOMER CODE:" from matching as a buyer label.
    const inlineLabelRx =
      /^(?:bill\s*to|billed\s*to|buyer|customer(?!\s+(?:code|id|number|#|type|account|ref))(?:\s*name)?|sold\s*to|ship\s*to|deliver\s*to|client|purchaser|account\s*holder)\s*[:.]?\s+(.+)/i;
    ...
    if (sectionStart >= 0) {
      // Limit buyer block to 5 lines and stop at any new section keyword
      const sectionStopRx =
        /^(?:invoice|tax\s+invoice|purchase\s+order|ship\s+to|deliver(?:y|ed)?\s+to|from|supplier|date|due\s+date|terms?|qty|description|item|total|subtotal|gst|tax)\b/i;
      const rawSlice = lines.slice(sectionStart, sectionStart + 5);
      const stopIdx = rawSlice.findIndex((l) => sectionStopRx.test(l));
      const buyerLines = stopIdx >= 0 ? rawSlice.slice(0, stopIdx) : rawSlice;
      const block = buyerLines.join("\n");
  ...
  private extractTransaction(
    ...
    if (invM) {
      ...
      // Try standalone header format: a line that is "INVOICE 2927935".
      // Use \b instead of $ so "INVOICE 12345 USD" still matches.
      // Require digit start to prevent "INVOICE SOLUTIONS INC" false-positive.
      const invHeaderRx = /^invoice\s+(\d[\w\-/]*\d|\d+)\b/im;
      const hdrM = fullText.match(invHeaderRx);
      if (hdrM?.[1]) {
        tx.invoiceNumber = hdrM[1].trim();
      } else {
        // Also try cash-sale reference format: "#CS129564" on its own line
        const csM = fullText.match(/^#([A-Z]{0,4}\d{4,12})$/im);
        if (csM?.[1]) tx.invoiceNumber = csM[1].trim();
      }
    }
    ...
    // PO number: PO-BR-4421, P.O. #4421, PO 12345, PO#: PO-2024-007
    ...
    // Use [#:] as the required delimiter for bare "PO" to avoid capturing label words like
    // "PO Number" as the PO value. For space-only delimiter ("PO 12345"), see fallback below.
    const poRx =
      /(?:p\.?o\.?[^\S\n]*[#:]+|purchase[^\S\n]*order[^\S\n]*(?:no\.?|#|number)[^\S\n]*[#:]*)[^\S\n]*:?[^\S\n]*([A-Z0-9][\w\-/]{1,30})/i;
    ...
    // "ORDER NUMBER" / "Order No." on one line, numeric value on next (HSC B2B invoice format)
    if (!tx.purchaseOrderNumber) {
      const orderLines = fullText.split("\n");
      for (let i = 0; i < orderLines.length - 1; i++) {
        if (/^\s*order\s+(?:no\.?|#|number)\s*:?\s*$/i.test(orderLines[i])) {
          const val = orderLines[i + 1].trim();
          if (val && /^\d[\w\-/]{0,20}$/.test(val)) {
            tx.purchaseOrderNumber = val;
            break;
          }
        }
    ...
    // Space-only PO delimiter fallback: "PO 12345" or "P.O. AB-1234" (no # or :).
    // Only fires when the value contains at least one digit (avoids capturing label
    // words like "Number" or "Date" that follow "PO " in column headers).
    if (!tx.purchaseOrderNumber) {
      const spacePOM = fullText.match(
        /\bp\.?o\.?[^\S\n]+([A-Z0-9][\w\-/]{1,30})/i,
      );
      const candidate = spacePOM?.[1]?.trim();
      if (candidate && /\d/.test(candidate)) {
        tx.purchaseOrderNumber = candidate;
      }
    }
    ...
    // Fallback: "Term Description" column header with value on same/next data row (HSC format)
    if (!tx.paymentTerms) {
      const blockTermM = fullText.match(
        /term\s*description[\s\S]{0,300}?\b(prepaid|net[-\s]?\d+\b|cod\b|cash\s*on\s*delivery|eft\b)/i,
      );
      if (blockTermM) tx.paymentTerms = blockTermM[1].trim();
    }
    ...
    // Reference number: Ref No. REF-2024-001, Our Ref: ABC123
    // Require a word boundary at start and a mandatory delimiter (:/-) to avoid
    // matching substrings like "REFID" which would capture "ID".
    const refM = fullText.match(
      /\b(?:ref(?:erence)?\s*(?:no\.?|#|number)?|our\s*ref)\s*[:\-]\s*([A-Z0-9][\w\-/]{1,30})/i,
    );
    if (refM?.[1]) tx.referenceNumber = refM[1].trim();

    // Work order number: WO-2024-001, Work Order #1234
    // Bare "wo" must be followed by an explicit delimiter (#/-/:) to avoid
    // matching "wo" fragments inside other words/phrases.
    const woM = fullText.match(
      /(?:work\s*order\s*(?:no\.?|#|number)?|(?:^|\s)wo\s*[#\-:])\s*[:\-]?\s*([A-Z0-9][\w\-/]{2,20})/im,
    );
    if (woM?.[1]) tx.workOrderNumber = woM[1].trim();

    // Invoice status: only match when it appears as an explicit labeled field
    // (e.g. "Status: PAID") or as a standalone stamp-style line (e.g. "OVERDUE").
    // Do NOT match "paid" embedded in "AMOUNT PAID:" or similar payment lines.
    const statusLabelM = fullText.match(
      /(?:^|\n)\s*(?:status|invoice\s*status)\s*[:\-]\s*(paid|unpaid|overdue|draft|issued|cancelled|canceled|void(?:ed)?)\b/i,
    );
    const statusStampM = !statusLabelM
      ? fullText.match(
          /(?:^|\n)\s*(overdue|void(?:ed)?|cancelled|canceled)\s*(?:\n|$)/i,
        )
      : null;
    const statusVal = (statusLabelM ?? statusStampM)?.[1];
    if (statusVal) {
      const sv = statusVal.toLowerCase();
      if (sv === "paid") tx.status = "paid";
      else if (sv === "unpaid" || sv === "issued") tx.status = "issued";
      else if (sv === "overdue") tx.status = "overdue";
      else if (sv === "draft") tx.status = "draft";
      else if (
        sv === "cancelled" ||
        sv === "canceled" ||
        sv === "void" ||
  ...
  private extractParts(
    ...
    // 4. Fallback: single-line heuristic on body lines (with non-product filter)
    ...
    // 5. Last resort: scan ALL lines but with STRICT filtering
    //    Only accept lines that have a plausible product name AND a
    //    price-like decimal number (e.g., "12.50", "$45.00").
    const allItems: DataLiftPart[] = [];
    const priceRx = /\$?\d+\.\d{2}/;
    allLines.forEach((line, idx) => {
      if (isNonProductLine(line)) return;
      // In the strict last-resort pass, require at least one price-like decimal
      if (!priceRx.test(line)) return;
      const item = parseLineItem(line, idx + 1);
  ...
  private extractFromColumnTable(lines: string[]): DataLiftPart[] {
    ...
    // Keywords that indicate a PRODUCT table (qty column must be present).
    // Headers without these (e.g. "PO Number  Term Description  Discount Amount")
    // are billing metadata tables and must not be used as the product table header.
    const qtyIndicatorKws = [
      "qty",
      "quantity",
      "ordered",
      "shipped",
    ...
    let headerIdx = -1; // first "weak" header (no qty indicator)
    let strongHeaderIdx = -1; // last "strong" header (has qty indicator)

    for (let i = 0; i < lines.length; i++) {
      const lower = lines[i].toLowerCase();
      const hits = headerKws.filter((kw) => lower.includes(kw)).length;
      if (hits >= 2) {
        const isStrong = qtyIndicatorKws.some((kw) => lower.includes(kw));
        if (isStrong) {
          strongHeaderIdx = i; // keep updating – use the LAST strong header
        } else if (headerIdx < 0) {
          headerIdx = i; // remember first weak header as fallback only
        }
      }
    }
    // Prefer strong (qty-indicator) header; fall back to weak only when no strong exists
    const resolvedHeaderIdx =
      strongHeaderIdx >= 0 ? strongHeaderIdx : headerIdx;
    if (resolvedHeaderIdx < 0) return [];
    // Reuse as `headerIdx` for the data-row loop below
    const effectiveHeaderIdx = resolvedHeaderIdx;
    ...
    for (let i = effectiveHeaderIdx + 1; i < lines.length; i++) {
      ...
      // Skip non-product lines (addresses, phone numbers, policy text, etc.)
      ...
      // Item names MUST have at least 3 consecutive alpha characters to be valid.
      // If itemName is too short (e.g. "EA" from a UOM column), look ahead to the
      // next line for a description — common in tabular invoices where item descriptions
      // are on a separate line below the qty/price row (HSC B2B invoice format).
      // Guard: only attempt look-ahead when:
      //   (a) the row has ≥2 numeric candidates AND
      //   (b) the last number (presumed total) has a fractional part, so that
      //       pure-date lines like "11/29/2023" (whose last token is the year 2023,
      ...
      if (itemName.length < 3 || !/[A-Za-z]{3,}/.test(itemName)) {
        ...
        if (candidateNums.length >= 2 && lastNumHasDecimal) {
          ...
          if (
            ...
            !isNonProductLine(nextLine)
          ) {
            itemName = nextLine;
            i++; // consume the description line so it isn't re-processed
          } else {
            continue; // no valid item name fallback, skip this row
          }
        } else {
          continue; // date-only or single-value line, not a product row
        }
      }
      // Also reject names that look like date fragments or short part codes
      if (/^\d{1,2}[\/\-]\d{1,2}[\/\-]?$/.test(itemName)) continue;
      // Re-check 3-alpha requirement after potential look-ahead replacement
      if (!/[A-Za-z]{3,}/.test(itemName)) continue;
      ...
      // Only consume the next line as a description if it does NOT look like
      // a product/item line itself (no currency symbol+number, no 2+ consecutive
      // digits, no standalone price-like decimal).
      let description: string | undefined;
      if (i + 1 < lines.length) {
        const nextLine = lines[i + 1].trim();
        const looksLikeItem =
          /\d{2,}/.test(nextLine) ||               // any 2+ consecutive digits (price/qty)
          /[$€£₹¥]\s*\d/.test(nextLine) ||         // currency symbol followed by digit
          /\b\d+\.\d{2}\b/.test(nextLine) ||       // price-like decimal (XX.XX)
          footerRx.test(nextLine);
        if (
          !looksLikeItem &&
          nextLine.length > 3 &&
      ...
      // Sanity guard: reject items whose extended total exceeds $1,000,000.
      // B2B bulk orders can legitimately have line items in the $100k-$1M range.
  ...
  private extractTotals(
    ...
    const tip = extractLabeledAmount(search, "tip|gratuity");
    const serviceCharge = extractLabeledAmount(
      search,
    ...
    // NOTE: only use "amount due" as grand total when it is > 0 (0 means already paid).
    let grandTotal =
      extractLabeledAmount(
        search,
      ...
      (() => {
        const due = extractLabeledAmount(
          search,
          "amount\\s+due|balance\\s+due",
        );
        return due !== undefined && due > 0 ? due : undefined;
      })() ??
      extractLabeledAmount(search, "amount\\s*:") ??
      extractLabeledAmount(
        footerSearch,
        "(?:invoice|invoice\\s+total|credit\\s+card|invoice\\s+89020)\\s*total|credit\\s*card",
      ) ??
      extractLabeledAmount(footerSearch, "\\btotal\\b") ??
      (() => {
        // Fallback: if subtotal is known, look for the largest standalone amount
        // in the footer that exceeds the subtotal (likely subtotal + tax = grand total).
        if (subtotal !== undefined && subtotal > 0) {
          const footerAmounts: number[] = [];
          const footLines = footerText.split("\n");
          for (const fl of footLines) {
            const amtM = fl.trim().match(/^[\$€£₹¥]?\s*([\d,]+\.\d{1,4})\s*$/);
            if (amtM?.[1]) {
              footerAmounts.push(parseFloat(amtM[1].replace(/,/g, "")));
            }
          }
          const largerThanSubtotal = footerAmounts
            .filter((a) => a > subtotal)
            .sort((a, b) => a - b);
          if (largerThanSubtotal.length > 0) return largerThanSubtotal[0];
        }
        return undefined;
      })() ??
      subtotal ??
    ...
    // ── Sanity: grandTotal == subtotal but footer has a larger amount ──────
    // This happens when the "\btotal\b" pattern matches "SUB-TOTAL" instead
    // of the real grand total. If a footer amount exceeds the subtotal
    // (likely subtotal + tax), use it as the grand total.
    if (
      subtotal !== undefined &&
      subtotal > 0 &&
      grandTotal === subtotal &&
    ...
    // ── Sanity check ──────────────────────────────────────────────────────
    // Grand total should never be much less than subtotal (unless heavy discount).
    // If it is, we likely picked up a line-item price or section total instead.
    // In that case, derive grand total from parts or fall back to subtotal.
    const partsSum = parseFloat(
      parts.reduce((s, p) => s + (p.totalAmount ?? 0), 0).toFixed(2),
    );
    if (
      grandTotal > 0 &&
      ...
      // Prefer parts-derived sum when available, otherwise subtotal
      grandTotal =
        partsSum > 0
          ? parseFloat(
              (
    ...
    // Also: if we have parts but no subtotal, derive it
    const resolvedSubtotal = subtotal ?? (partsSum > 0 ? partsSum : undefined);

    let amountPaid = extractLabeledAmount(
      search,
      "amount\\s+paid|cash\\s+tendered|tendered|credit\\s*card",
    );
    let balanceDue = extractLabeledAmount(
      search,
      "balance\\s+due|change\\s+due|amount\\s+due",
    );

    // Derive amountPaid / balanceDue from grand total when one is known but the other isn't.
    // Common case: credit card payment makes balance 0 and paid = grand total.
    if (
      ...
      // Column-separated mapping shifted amounts — credit card shows 0 but
      // the grand total was correctly found. Fix: paid = grand total.
      ...
      balanceDue = parseFloat((grandTotal - amountPaid).toFixed(2));
    } else if (
      amountPaid === undefined &&
      balanceDue !== undefined &&
      balanceDue >= 0 &&
      ...
      amountPaid = parseFloat((grandTotal - balanceDue).toFixed(2));
    }

    // If both amountPaid and balanceDue are still undefined but a payment method
    // was detected (e.g. credit card) and we have a grand total, infer the full
    // amount was paid and balance is zero.
    if (amountPaid === undefined && balanceDue === undefined && grandTotal > 0) {
      const hasPayment =
        /\b(?:mastercard|visa|amex|discover|credit\s*card|debit\s*card|paid|cash|check|cheque)\b/i.test(
          fullText,
        );
      if (hasPayment) {
        amountPaid = grandTotal;
    ...
    // Derive totalTax from grandTotal − subtotal when tax label was present
    // but no numeric value was extracted (common in column-separated OCR).
    if (
      totalTax === undefined &&
      resolvedSubtotal !== undefined &&
      resolvedSubtotal > 0 &&
      grandTotal > resolvedSubtotal
    ) {
      const hasTaxLabel = /\b(?:tax|gst|vat)\b/i.test(fullText);
      if (hasTaxLabel) {
        totalTax = parseFloat((grandTotal - resolvedSubtotal - (shippingCost ?? 0) + (discount ?? 0)).toFixed(2));
      }
    }

    // Sanity check: totalTax must be plausible relative to the grand total.
    // If it is more than 60% of the grand total, or is absurdly large (> $10 000),
    // we likely picked up an invoice number or other unrelated large number.
    if (
      totalTax !== undefined &&
      ((grandTotal > 0 && totalTax > grandTotal * 0.6) || totalTax > 10_000)
    ) {
      totalTax = undefined;
    }
