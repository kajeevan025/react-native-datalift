...
/**
 * Lines that are clearly NOT product/service entries.
...
export function isNonProductLine(line: string): boolean {
  return NON_PRODUCT_LINE_RX.test(line.trim());
}
...
export function parseLineItem(
  ...
  while ((m = numPattern.exec(trimmed)) !== null) {
    // Group 2: comma-formatted integer part, Group 3: plain integer part, Group 4: decimal part
  ...
  const uomM = trimmed.match(PATTERNS.UOM);
...
const CARD_BRAND_RX =
  /(?:^|\s)(visa|mastercard|master\s+card|amex|american\s+express|eftpos|diners|discover|jcb)(?:\s|$)/im;
const CARD_LAST4_RX =
  /(?:card|a\/c|account)\s*(?:ending|ends|last\s*4|no\.?)?\s*[#*x]{0,4}(\d{4})\b/i;
const BSB_RX = /\bBSB\s*[:#.]?\s*([0-9]{3}[-\s]?[0-9]{3})\b/i;
const BANK_ACCOUNT_RX =
  /\b(?:acc(?:ount)?|a\/c)\s*(?:no\.?|#|number)?[:#.]?\s*([*xX0-9]{4,18})\b/i;
const RECEIPT_NO_RX =
  /(?:receipt|authoris(?:ation)?|authori[sz]ation|auth(?:orisation)?|approval|rrn|approval\s*code)\s*(?:no\.?|#|number)?[:#.]?\s*([A-Z0-9][\w\-]{1,30})/i;

export function extractPaymentDetails(
  text: string,
): DataLiftPaymentDetails | undefined {
  const pd: DataLiftPaymentDetails = {};

  // Payment method — labeled
  const methodRx =
    /(?:payment\s*(?:method|mode|type|via|by|tender)|paid\s*(?:via|by|with))\s*[:\-]?\s*([A-Za-z][A-Za-z\s\/]{2,25})/i;
  const methodM = text.match(methodRx);
  if (methodM?.[1]) {
    pd.method = methodM[1].trim().replace(/\s+/g, " ");
  }
  // Fallback: common method keywords inline
  if (!pd.method) {
    const kwM = text.match(
      /\b(cash|eftpos|credit\s+card|debit\s+card|bank\s+transfer|eft|bpay|paypal|cheque|check|direct\s+debit|direct\s+credit|wire\s+transfer)\b/i,
    );
    if (kwM)
      pd.method = kwM[1]
        .replace(/\s+/g, " ")
  ...
  // Card brand
  const brandM = text.match(CARD_BRAND_RX);
  if (brandM?.[1])
    pd.cardType =
      brandM[1].charAt(0).toUpperCase() + brandM[1].slice(1).toLowerCase();

  // Card last 4 digits
  const last4M = text.match(CARD_LAST4_RX);
  if (last4M?.[1]) pd.cardLast4 = last4M[1];

  // AU bank BSB
  const bsbM = text.match(BSB_RX);
  if (bsbM?.[1]) pd.bankBsb = bsbM[1].replace(/\s/g, "-");

  // Bank account number
  const bankM = text.match(BANK_ACCOUNT_RX);
  if (bankM?.[1] && !BSB_RX.test(bankM[1])) pd.bankAccount = bankM[1];

  // Receipt / authorisation number
  const receiptM = text.match(RECEIPT_NO_RX);
  if (receiptM?.[1]) pd.receiptNumber = receiptM[1].trim();

  // Payment reference number
  if (!pd.reference) {
    const refRx =
      /(?:payment\s*ref(?:erence)?|pay\s*ref|ref(?:erence)?\s*no\.?)\s*[:#.]?\s*([A-Z0-9][\w\-]{1,30})/i;
    const refM = text.match(refRx);
...
export function extractDeliveryDetails(
  text: string,
): DataLiftDeliveryDetails | undefined {
  const dd: DataLiftDeliveryDetails = {};

  // Ship-to / deliver-to address block (greedy up to a blank line or section header)
  const shipRx =
    /(?:ship\s*(?:to|ping\s*address)|deliver(?:y\s*(?:to|address)|ed?\s*to)?|consignee)\s*[:.]?\s*\n?([\s\S]{5,300}?)(?=\n\n|\n[A-Z][A-Z\s]+[:\n]|\n(?:invoice|date|terms|sub\s*total|total|tax)\b|$)/i;
  const shipM = text.match(shipRx);
  if (shipM?.[1]) {
    const addrBlock = shipM[1].trim();
    if (addrBlock.length > 5) dd.address = parseAddress(addrBlock);
  }

  // Delivery / dispatch date
  const delivDateRx =
    /(?:delivery|dispatch|ship(?:ment)?|despatch(?:ed)?)\s*date\s*[:\-]?\s*(\d{1,4}[-/.]\d{1,2}[-/.]\d{2,4})/i;
  const delivDateM = text.match(delivDateRx);
  if (delivDateM?.[1]) dd.date = delivDateM[1];
  ...
  if (carrierM?.[1])
    dd.carrier = carrierM[1]
      .replace(/\s+/g, " ")
      .replace(/\b\w/g, (c) => c.toUpperCase());

  // Shipping method
  const methodRx =
    /(?:shipping|delivery|dispatch)\s*(?:method|service|type|via)\s*[:\-]?\s*([A-Za-z][A-Za-z\s]{2,30})/i;
  const methodM = text.match(methodRx);
  if (methodM?.[1]) dd.shippingMethod = methodM[1].trim().replace(/\s+/g, " ");
...
export function extractNotes(text: string): string | undefined {
  // Labeled block: "Notes:", "Remarks:", "Comments:", "Special Instructions:", etc.
  const blockRx =
    /(?:^|\n)\s*(?:notes?|remarks?|comments?|special\s+instructions?|instructions?|memo|additional\s+info(?:rmation)?)\s*[:\-]\s*\n?([\s\S]{5,500}?)(?=\n\n|\n[A-Z][A-Z\s]*[:\n]|\n(?:sub\s*total|total|tax|payment)\b|$)/im;
  const blockM = text.match(blockRx);
  if (blockM?.[1]) {
    const notes = blockM[1].trim().replace(/\n{3,}/g, "\n\n");
    if (notes.length >= 5) return notes;
  }

  // Inline: "Note: Please ensure delivery before 5pm"
  const inlineRx =
    /(?:^|\n)\s*(?:note|important|please\s+note)[:\s]+(.{10,200})/im;
  const inlineM = text.match(inlineRx);
