...
interface DataLiftConfig {
  ...
  /**
   * When true, automatically download the LayoutLMv3 model on first extract()
   * if it is not already present in local storage.
   */
  autoDownloadLayoutLMv3: boolean;
  /**
   * Optional custom base URL for downloading LayoutLMv3 model assets.
...
export interface DataLiftConfigureOptions {
  ...
  /**
   * When true the SDK will automatically attempt to download the LayoutLMv3
   * model on the first `extract()` call if it is not already present.
   * Downloading happens in the background and does not block OCR or
   * rule-based extraction — results improve once the model is cached.
   * @default false
   */
  autoDownloadLayoutLMv3?: boolean;
  /**
   * Override the base URL from which LayoutLMv3 model assets are downloaded.
   * Useful for enterprise environments with internal model registries.
   */
  layoutLMv3ModelUrl?: string;
  /**
   * Optional callback that receives download progress when the LayoutLMv3
   * model is being auto-downloaded.
   */
...
export const DataLiftSDK = {
  ...
  configure(options: DataLiftConfigureOptions): void {
    ...
    if (options.autoDownloadLayoutLMv3 !== undefined) {
      _config.autoDownloadLayoutLMv3 = options.autoDownloadLayoutLMv3;
    }
  ...
  /**
   * Explicitly ensures the LayoutLMv3 model is present on device.
   *
   * Call this at app startup (or before the first `extract()`) to:
   *   1. Check if the model already exists in local storage.
   *   2. Download it from GitHub Releases if `autoDownload` is true.
   *   3. Auto-configure the native engine with the resolved paths.
   *
   * @example
   *   await DataLift.prepareModel({
   *     autoDownload: true,
   *     onProgress: ({ fileName, progress }) =>
   *       console.log(`Downloading ${fileName}: ${(progress * 100).toFixed(0)}%`),
   *   });
   */
  async prepareModel(
    options: EnsureModelOptions = {},
  ): Promise<{ ready: boolean; source?: string; modelPath?: string }> {
    try {
      const paths = await defaultModelManager.ensureModel(options);
      if (!paths) return { ready: false };

      // Auto-configure the native engine with the resolved paths
      if (
        !_layoutLMv3Config.configured ||
        _layoutLMv3Config.modelPath !== paths.modelPath
      ) {
        await this.configureLayoutLMv3({
          modelPath: paths.modelPath,
          labelsPath: paths.labelsPath,
        });
      }
      return { ready: true, source: paths.source, modelPath: paths.modelPath };
    } catch {
      return { ready: false };
    }
  },
  ...
  async extract(options: DataLiftExtractOptions): Promise<DataLiftResponse> {
    ...
    // ── 5.5. LayoutLMv3 on-device enhancement ───────────────────────────────
    //
    // Priority order for model resolution:
    //   1. Caller passed an explicit absolute path in options
    //   2. configureLayoutLMv3() was called at startup → use global config
    //   3. Auto-discover: check if model was previously downloaded to storage
    //   4. Auto-download: if autoDownloadLayoutLMv3=true and model not found
    //
    // OCR + rule-based extraction has already run above; LayoutLMv3 fills gaps.
    // ─────────────────────────────────────────────────────────────────────────
    ...
    // Attempt auto-discovery / auto-download when model isn't already configured
    if (!_layoutLMv3Config.configured && !hasValidExplicitPath) {
      const shouldDownload =
        (options as any).autoDownloadLayoutLMv3 ??
        _config.autoDownloadLayoutLMv3;

      try {
        const modelUrls = _config.layoutLMv3ModelUrl
        ...
        if (discovered) {
          logger.info(
            `LayoutLMv3 model auto-discovered (source=${discovered.source}) – configuring`,
          );
          // Silently configure the native engine (non-throwing)
          try {
            await this.configureLayoutLMv3({
              modelPath: discovered.modelPath,
              labelsPath: discovered.labelsPath,
            });
          } catch (configErr) {
            logger.warn(
              `Auto-configure LayoutLMv3 failed (non-fatal): ${configErr instanceof Error ? configErr.message : String(configErr)}`,
            );
          }
        } else if (shouldDownload) {
          logger.info(
            "LayoutLMv3 model not yet available – download triggered, will apply on next extract()",
          );
          ...
          defaultModelManager
            ...
            .catch((err) => {
              logger.warn(
                `Background LayoutLMv3 download failed: ${err instanceof Error ? err.message : String(err)}`,
              );
            });
        } else {
          logger.debug(
            "LayoutLMv3 model not present and autoDownload=false – skipping",
          );
        }
      } catch (discoverErr) {
        logger.debug(
          `LayoutLMv3 model discovery failed (non-fatal): ${discoverErr instanceof Error ? discoverErr.message : String(discoverErr)}`,
        );
      }
    }
