...
class DataLiftModule(reactContext: ReactApplicationContext) :
    ...
    private fun resolveLabelsPath(labelsPathOverride: String?): String? {
        return if (!labelsPathOverride.isNullOrBlank()) {
    ...
    fun configureLayoutLMv3(options: ReadableMap, promise: Promise) {
        scope.launch {
            try {
                ...
                // Pre-extract vocab.json and merges.txt alongside the model so
                // LayoutLMv3OnnxEngine.loadVocab(modelFile.parentFile) can find them.
                // resolveBundlePath extracts from APK assets → filesDir/models/<name>
                // which is the same parent directory as the resolved model file.
                val modelDir = java.io.File(modelPath).parentFile
                if (modelDir != null) {
                    for (vocabFile in listOf("vocab.json", "merges.txt")) {
                        val dest = java.io.File(modelDir, vocabFile)
                        if (!dest.exists()) {
                            resolveBundlePath(vocabFile) // extracts to filesDir/models/
                        }
                    }
                }
    ...
    fun predictLayoutLMv3(options: ReadableMap, promise: Promise) {
        scope.launch {
            try {
                ...
                // Load bitmap from image_uri (primary) or legacy uris[] (fallback)
                val imageBitmap = options.getString("image_uri")
                    ?.let { uri -> runCatching { loadBitmap(uri) }.getOrNull() }
                    ?: options.getArray("uris")
                        ?.takeIf { it.size() > 0 }
                        ?.getString(0)
                        ?.let { uri -> runCatching { loadBitmap(uri) }.getOrNull() }
    ...
    fun checkLayoutLMv3Compatibility(options: ReadableMap, promise: Promise) {
        scope.launch {
            ...
            try {
                ...
                val labelsExists = labelsPath?.let { java.io.File(it).exists() } ?: false
                ...
                if (labelsPath != null && !labelsExists) {
                    throw IllegalArgumentException("LayoutLMv3 labels file not found at path: $labelsPath")
                }
                ...
                promise.resolve(WritableNativeMap().apply {
                    ...
                    putString("runtime", prediction.runtime)
                    putString("model_path", modelPath)
                    if (labelsPath != null) putString("labels_path", labelsPath)
