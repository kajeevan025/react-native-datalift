...
class DataLiftModule(reactContext: ReactApplicationContext) :
    ...
    private fun resolveModelPath(modelPathOverride: String?): String {
        return if (!modelPathOverride.isNullOrBlank()) {
            filePathFromUri(modelPathOverride)
                ?: throw IllegalArgumentException("LayoutLMv3 model '$modelPathOverride' not found.")
        } else {
            layoutLMv3ModelPath
    ...
    private fun resolveLabelsPath(labelsPathOverride: String?): String {
        val labelsPath = if (!labelsPathOverride.isNullOrBlank()) {
            filePathFromUri(labelsPathOverride)
                ?: throw IllegalArgumentException("LayoutLMv3 labels file '$labelsPathOverride' not found.")
        } else {
            ...
            try {
                ...
                if (modelPath == null) {
                    ...
                    throw IllegalArgumentException(
                        "LayoutLMv3 model '$modelPathRaw' not found. " +
                        "Place the .onnx model in one of: " +
                        "android/app/src/main/assets/, " +
                        "${ctx.filesDir.absolutePath}/, " +
                        "or a 'models/' subdirectory of either location."
                    )
                }
                val modelFile = java.io.File(modelPath)
                if (!modelFile.exists()) {
                    throw IllegalArgumentException("LayoutLMv3 model resolved but not found at path: $modelPath")
                }
                ...
                if (!labelsPathRaw.isNullOrBlank()) {
                    ...
                    if (labelsPath == null) {
                        throw IllegalArgumentException("LayoutLMv3 labels file '$labelsPathRaw' not found in app bundle or assets.")
                    }
    ...
    /**
     * Returns the directory used to store downloaded LayoutLMv3 model files.
     * Creates the directory if it does not yet exist.
     * Path: <filesDir>/DataLift/models/layoutlmv3/
     */
    @ReactMethod
    fun getModelStorageDir(promise: Promise) {
        scope.launch {
            try {
                val modelDir = java.io.File(
                    reactApplicationContext.filesDir,
                    "DataLift/models/layoutlmv3"
                )
                if (!modelDir.exists()) modelDir.mkdirs()
                promise.resolve(WritableNativeMap().apply {
                    putString("path", modelDir.absolutePath)
                })
            } catch (e: Exception) {
                Log.e(TAG, "getModelStorageDir error", e)
                promise.reject("MODEL_DIR_ERROR", e.message, e)
            }
        }
    }
     ...
     * HttpURLConnection request. Safe for large model files.
     *
     * @param options ReadableMap with "url" and "destination" keys
     * @param promise resolves with { path: String, bytes: Int }
     */
    @ReactMethod
    fun downloadModelFile(options: ReadableMap, promise: Promise) {
        scope.launch {
            try {
                ...
                if (responseCode !in 200..299) {
                    ...
                    throw IllegalStateException("Server returned HTTP $responseCode for $urlString")
                }
                ...
                val bytes = destFile.length().toInt()
                promise.resolve(WritableNativeMap().apply {
                    putString("path", destFile.absolutePath)
                    putInt("bytes", bytes)
                })
            } catch (e: Exception) {
                Log.e(TAG, "downloadModelFile error", e)
