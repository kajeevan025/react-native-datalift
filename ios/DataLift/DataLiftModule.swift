...
class DataLiftModule: NSObject, RCTBridgeModule {
    ...
    private func resolveLabelsPath(_ labelsPathOverride: String?) -> String? {
        ...
        if let override = trimmedOverride, !override.isEmpty {
            return filePathFromUri(override)
        }
        return layoutLMv3LabelsPath
    }
    ...
    func predictLayoutLMv3(
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                // Load image from image_uri if provided
                var inputImage: UIImage? = nil
                if let imageUri = options["image_uri"] as? String,
                   let resolved = filePathFromUri(imageUri) {
                    inputImage = UIImage(contentsOfFile: resolved)
                }
                let prediction = try self.predictLayoutLMv3Internal(
                    rawText: rawText,
                    modelPathOverride: options["model_path"] as? String,
    ...
    func checkLayoutLMv3Compatibility(
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                // ::: Code Generated by Copilot b7e3a21f-4c8d-4b9e-a5f1-2d8e9c0f3a7b. This comment will be removed automatically after the file is saved :::
                let labelsPath = self.resolveLabelsPath(options["labels_path"] as? String)

                let modelExists = FileManager.default.fileExists(atPath: modelPath)
                let labelsExists = labelsPath.map { FileManager.default.fileExists(atPath: $0) } ?? false
                ...
                // Labels are optional — engine uses built-in label map when not provided
                if let labelsPath, !labelsExists {
                ...
                if let labelsPath { resolveDict["labels_path"] = labelsPath }
                resolve(resolveDict)
            } catch {
                resolve([
                    "compatible": false,
    ...
    private func predictLayoutLMv3Internal(
        ...
        // ::: Code Generated by Copilot b7e3a21f-4c8d-4b9e-a5f1-2d8e9c0f3a7b. This comment will be removed automatically after the file is saved :::
        let labelsPath = resolveLabelsPath(labelsPathOverride)
        if let labelsPath {
    ...
    func downloadModelFile(
        ...
        // ::: Code Generated by Copilot f9a2c4e1-3b7d-4f8a-9c2e-5d1b0e6a8f3c. This comment will be removed automatically after the file is saved :::
        let task = URLSession.shared.downloadTask(with: url) { tmpURL, response, error in
            ...
            // Validate HTTP status — URLSession does NOT throw on 4xx/5xx responses.
            // Without this check a GitHub 404 HTML page gets saved as the model file,
            // which then causes "EOCD record not found" when the extractor tries to unzip it.
            if let http = response as? HTTPURLResponse, !(200...299).contains(http.statusCode) {
                // Clean up the temp download (non-throwing)
                if let tmpURL { try? FileManager.default.removeItem(at: tmpURL) }
                reject(
                    "DOWNLOAD_ERROR",
                    "Server returned HTTP \(http.statusCode) for \(urlString). " +
                    "Verify the model release exists at the configured URL.",
                    nil
                )
                return
            }
            ...
            do {
                // Remove any previous version at the destination (could be a corrupt file
                // from a previous failed download)
                ...
                // Remove partial file to avoid future "EOCD not found" errors
                try? FileManager.default.removeItem(at: destURL)
