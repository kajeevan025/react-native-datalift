...
class DataLiftModule: NSObject, RCTBridgeModule {
     ...
     * For bare filenames (no "/" prefix), searches Bundle.main resources.
     * Returns nil when a bare filename cannot be resolved (instead of returning the bare name).
     */
    private func filePathFromUri(_ uri: String) -> String? {
        ...
        // Bare filename — try to resolve from app bundle
        return resolveBundlePath(uri)
    }

    /**
     * Search for a file inside Bundle.main by name (with or without extension).
     * Checks: exact name in resources, name split into (stem, ext), and
     * common model subdirectories.
     * For .onnx files also tries .mlmodelc (compiled CoreML) and .mlpackage variants.
     */
    private func resolveBundlePath(_ filename: String) -> String? {
        // 1. Try direct path inside bundle
        let bundleRoot = Bundle.main.bundlePath
        let directPath = (bundleRoot as NSString).appendingPathComponent(filename)
        if FileManager.default.fileExists(atPath: directPath) {
            return directPath
        }

        // 2. Try Bundle.main.path(forResource:ofType:)
        let nsName = (filename as NSString)
        let ext = nsName.pathExtension
        let stem = nsName.deletingPathExtension
        if !ext.isEmpty, let found = Bundle.main.path(forResource: stem, ofType: ext) {
            return found
        }
        // Try without splitting extension
        if let found = Bundle.main.path(forResource: filename, ofType: nil) {
            return found
        }

        // 3. For .onnx files, also try CoreML variants (.mlmodelc, .mlpackage)
        //    since iOS uses CoreML for inference, not ONNX Runtime directly
        let lowExt = ext.lowercased()
        if lowExt == "onnx" || lowExt == "bin" {
            for altExt in ["mlmodelc", "mlpackage", "mlmodel"] {
                let altName = stem + "." + altExt
                let altDirect = (bundleRoot as NSString).appendingPathComponent(altName)
                if FileManager.default.fileExists(atPath: altDirect) {
                    return altDirect
                }
                if let found = Bundle.main.path(forResource: stem, ofType: altExt) {
                    return found
                }
            }
        }

        // 4. Search common subdirectories
        let subdirs = ["models", "assets", "model", "ml"]
        let candidates = [filename]
            + (lowExt == "onnx" || lowExt == "bin"
                ? [stem + ".mlmodelc", stem + ".mlpackage", stem + ".mlmodel"]
                : [])

        for dir in subdirs {
            let subPath = (bundleRoot as NSString).appendingPathComponent(dir)
            for candidate in candidates {
                let filePath = (subPath as NSString).appendingPathComponent(candidate)
    ...
    private func loadImage(from uri: String) throws -> UIImage {
        guard let filePath = filePathFromUri(uri) else {
            throw DataLiftError.invalidInput("Cannot resolve file path for URI: \(uri)")
        }
    ...
    func configureLayoutLMv3(
        ...
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            ...
            do {
                ...
                guard let resolvedModelPath = self.filePathFromUri(modelPath) else {
                    let bundleRoot = Bundle.main.bundlePath
                    throw DataLiftError.invalidInput(
                        "LayoutLMv3 model '\(modelPath)' not found. " +
                        "Add the model file to your Xcode project's bundle resources. " +
                        "iOS uses CoreML — if you have an .onnx file, convert it with coremltools first. " +
                        "Searched: \(bundleRoot) and subdirs [models/, assets/, model/, ml/]. " +
                        "Also tried .mlmodelc/.mlpackage variants."
                    )
                }
                guard FileManager.default.fileExists(atPath: resolvedModelPath) else {
                    throw DataLiftError.invalidInput("LayoutLMv3 model resolved but not found at path: \(resolvedModelPath)")
                }
                ...
                if let labelsPath, !labelsPath.isEmpty {
                    guard let resolvedLabelsPath = self.filePathFromUri(labelsPath) else {
                        throw DataLiftError.invalidInput("LayoutLMv3 labels file '\(labelsPath)' not found in app bundle.")
                    }
    ...
    private func buildSchemaFromModelPrediction(
        ...
        let imagesPayload: [[String: Any]] = uris.enumerated().map { index, uri in
            ...
            if index < fileNames.count, !fileNames[index].isEmpty {
                ...
                fileName = URL(fileURLWithPath: filePathFromUri(uri) ?? uri).lastPathComponent
            }
            return [
    ...
    // MARK: - getModelStorageDir

    /**
     * Returns the platform-specific directory used to store downloaded LayoutLMv3 model files.
     * Creates the directory if it does not yet exist.
     * Path: <App>/Library/Application Support/DataLift/models/layoutlmv3/
     */
    @objc
    func getModelStorageDir(
        ...
        DispatchQueue.global(qos: .utility).async {
            do {
                guard let appSupportUrl = FileManager.default.urls(
                    ...
                    throw DataLiftError.invalidInput("Cannot access Application Support directory")
                }
                let modelDir = appSupportUrl
                    .appendingPathComponent("DataLift")
                    .appendingPathComponent("models")
                    .appendingPathComponent("layoutlmv3")
                ...
                reject("MODEL_DIR_ERROR", error.localizedDescription, error)
            }
        }
    }

    // MARK: - downloadModelFile

    /**
     * Downloads a remote file to a local destination path using a streaming
     * URLSession download task. Safe for large model files (no in-memory buffering).
     *
     * - Parameter options: { url: String, destination: String }
     * - Returns: { path: String, bytes: Int }
     */
    @objc
    func downloadModelFile(
        ...
        guard let urlString = options["url"] as? String,
            ...
            reject("DOWNLOAD_ERROR", "'url' and 'destination' are required", nil)
            return
        }
        ...
        let task = URLSession.shared.downloadTask(with: url) { tmpURL, response, error in
            if let error = error {
                reject("DOWNLOAD_ERROR", "Download failed: \(error.localizedDescription)", error)
                return
            }
            guard let tmpURL = tmpURL else {
                reject("DOWNLOAD_ERROR", "Download produced no temporary file", nil)
                return
            }
            do {
                // Remove any previous version at the destination
                ...
                resolve(["path": destination, "bytes": bytes])
            } catch {
                reject("DOWNLOAD_ERROR", "Failed to save downloaded file: \(error.localizedDescription)", error)
            }
        }
        task.resume()
    }
