  ...
  const processUri = useCallback(
    async (uri: string) => {
      ...
      try {
        ...
        //  2. RuleBasedParser
        //  3. LayoutLMv3 native prediction (step 5.5, only when model is configured)
        const result = await DataLift.extract({
          image: uri,
          language: "eng",
          debug: __DEV__,
          // Only pass model paths if configuration succeeded — avoids
          // "model path does not exist" warning on every extraction
          ...(modelConfigured
            ? { layoutLMv3ModelPath, layoutLMv3LabelsPath }
            : {}),
        });
    ...
    [modelConfigured, layoutLMv3ModelPath, layoutLMv3LabelsPath],
  );
